...
public class ConwaysApplication extends JFrame implements MouseInputListener, Runnable {
    ...
    public void paint(Graphics g) {
        if (isGraphicsInstantiated) {
            ...
            if (!isGamePlaying) { // painting buttons
                g.setColor(Color.WHITE);
                g.fillRect(10, 35, 400, 40);
                g.setColor(Color.GREEN);
                g.fillRect(15, 40, 90, 30);
                g.fillRect(115, 40, 90, 30);
                g.fillRect(215, 40, 90, 30);
                g.fillRect(315, 40, 90, 30);

                g.setColor(Color.BLUE);
                g.setFont(new Font("Arial", Font.BOLD, 17));
                g.drawString("START", 25, 60);
                g.drawString("RANDOM", 120, 60);
                g.drawString("SAVE", 235, 60);
                g.drawString("LOAD", 335, 60);
            ...
            if (isGamePlaying) {
                ...
                for (int x = 0; x < 40; x++) {
                    for (int y = 0; y < 40; y++) {
                        ...
                        if (count < 2 && gamePointsBool[x][y][0])
                            gamePointsBool[x][y][1] = false;
                        else if ((count == 2 || count == 3) && gamePointsBool[x][y][0])
                            gamePointsBool[x][y][1] = true;
                        else if (count > 3 && gamePointsBool[x][y][0])
                            gamePointsBool[x][y][1] = false;
                        else if (!gamePointsBool[x][y][0] && count == 3)
                            gamePointsBool[x][y][1] = true;
                    }
                }
                for (int i = 0; i < cells; i++) {
    ...
    public void save() {
        String filename = "C:\\Users\\catha\\Desktop\\testing\\lifegame.txt";
                ...
                try {
                    for (int x = 0; x < 40; x++) {
                        ...
                        for(int y = 0; y < line.length(); y++) {
                            if(line.charAt(y) == '1') {
                                gamePointsBool[x][y][0] = true;
                            } else {
                                gamePointsBool[x][y][0] = false;
                            }
                        }
                    }
                } catch (IOException e) {}
    ...
    public void mousePressed(MouseEvent e) {
        ...
        // 215, 40, 90, 30)
        if (e.getX() > 210 && e.getX() < 310 && e.getY() < 70) {
            save();
        }
        // 315, 40, 90, 30
        if (e.getX() > 310 && e.getX() < 410 && e.getY() < 70) {
            load();
        }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        gamePointsBool[x][y][0] = true; // change the cell to the inverse of itself
